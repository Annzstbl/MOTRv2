...
# 检查OUTPUT_DIR是否存在，不存在则创建
if [ ! -d "${OUTPUT_DIR}" ]; then
    mkdir -p "${OUTPUT_DIR}"
fi

echo ${EXP_DIR} >> ${OUTPUT_DIR}/predict.log
echo ${GPU} >> ${OUTPUT_DIR}/predict.log
...
CUDA_VISIBLE_DEVICES=${GPU} python3 submit_hsmot_8ch.py \
    ...
    --query_denoise 0.05 \
    --num_queries 10 \
    --det_db ../workdir/motrv2/rebuttal_dota_test.json \
    --use_checkpoint \
    --mot_path ../data/HSMOT \
    --output_dir ${OUTPUT_DIR} \
    --input_channels 8 \
    | tee -a ${OUTPUT_DIR}/predict.log
